...
** Defining and constructing tables
...
*** TODO Specing expected slots in the metatable
...
*** TODO Expand {<metatable>} constructor into #%table

Expand into ~#%table~, which we expose and let the user override. This is in
addition to overriding :new metamethod.
...
send, send/self - which by default imply get, but get doesn't imply invocation.

proc - table used as procedure, probably indirectly referenced from prop:proc?
...
*** TODO t.key t:key t..key t::key

*** TODO [#C] . and : in app position
...
** TODO Table type must be able "to Racket"
...
** TODO Replace rawget/set with gen:dict or gen:associative
...
** TODO Basic Lua-style dispatch as gen:table
...
** TODO Multiple inheritance with <tables> metatable

Prototype precedence probably by overriding +gen:table+ (nope, can't be Racket
generics, cause we are not redefining tables, just using a different mt, so ought to be
changing metatable methods or multimethods) methods, call-next-method or send-next
bound within method's body, :before and :after combinations.
...
1. Metatable (prototype) dispatch,
2. Single argument "type" (metatable) dispatch,
3. Compute dispatch value for /isa/ multimethod dispatch,
4. Compute dispatch value for /implies/ predicate dispatch.
...
*** TODO Metatable (aka prototype) dispatch

Our base - bread and butter - dispatch: follow prototype chain until key or method
is found.

*** TODO <generic> metatable for Nth arg metatable dispatch
...
**** TODO Predefined <generic> methods e.g. get, set
