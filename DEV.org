...
*** TODO consider <set> metamethod semantics

We may want to offer <set> if only to allow <spec> guards whenever the value's
modified. <set> takes 3 arguments: self (table), key and a new value. Since the
...
*** TODO consider <name> metamethod semantics
...
*** TODO Consider delegating undefined guard to ~set~
...
*** TODO Allow #:kw args in {} constructors
...
*** TODO <spec> metatable and possibly #:spec keyword arg
...
3. potentially guard slots when they are inserted or set (which hints that just
   having <insert> may not be enough and <set> is indeed required).
...
#+begin_src racket
  ...
  ;; <setmeta>, <set> and <insert> which would be the same every time.
...
*** IDEA #:closed or #:sealed arg
CLOSED: [2019-06-02 Sun 09:49]

Tables default to be open maps. #:closed would signal that only declared or speced
slots are allowed.
